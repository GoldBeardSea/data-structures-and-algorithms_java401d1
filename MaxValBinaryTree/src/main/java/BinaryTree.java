...
public class BinaryTree {
    ...
    public int findMax(TreeNode traversingNode) {
        if (traversingNode == null) {
            return Integer.MIN_VALUE;
        }
        int result = traversingNode.getData();
        int leftTraverse = findMax(traversingNode.left);
        int rightTraverse = findMax(traversingNode.right);

        if (leftTraverse > result) {
            result = leftTraverse;
        }
        if (rightTraverse > result) {
            result = rightTraverse;
        }
        return result;
    }
